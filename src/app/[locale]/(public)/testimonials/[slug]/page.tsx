...
async function getTestimonial(slug: string) {
  const testimonial = await testimonialService.getById(slug).catch(() => testimonialService.getById(slug));
...
export default async function PublicTestimonialDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const testimonial: any = await getTestimonial(slug).catch(() => notFound());
