...
export default function AdminGalleriesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure? This will permanently delete this gallery.")) {
      await deleteGallery.mutateAsync(id);
    }
  };

  if (error) {
    return <StateScreen state="error" title="Unable to load galleries" compact />;
  }
  ...
  return (
    <div className="ui-reveal-up w-full space-y-6 select-none">
      <AdminPageBanner
        icon={Images}
        title={<I18n>Galleries</I18n>}
        description={<I18n>Manage visual collections and the stories behind each gallery.</I18n>}
        actions={
          <Link href={`/${locale}/admin/galleries/create`}>
      ...
      <DataTableEngine
        ...
        onView={(item) => router.push(`/${locale}/admin/galleries/${item.id}`)}
        onEdit={(item) => router.push(`/${locale}/admin/galleries/${item.id}/edit`)}
        ...
        bulkActions={[
          {
            ...
            onClick: async (selected) => {
              if (confirm(`Delete ${selected.length} selected galleries?`)) {
                await Promise.all(selected.map((item) => deleteGallery.mutateAsync(item.id)));
              }
            },
          },
        ]}
