...
import { useClients, useDeleteClient, useClientStore, clientTableColumns } from "@/features/client";
...
export default function AdminClientsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure? This will permanently delete this client.")) {
      await deleteClient.mutateAsync(id);
    }
  };

  if (error) {
    return <StateScreen state="error" title="Unable to load clients" compact />;
  }
  ...
  return (
    <div className="ui-reveal-up w-full space-y-6 select-none">
      <AdminPageBanner
        icon={ImageIcon}
        title={<I18n>Clients</I18n>}
        description={<I18n>Manage client logos, website links, and display order.</I18n>}
        actions={
          <Link href={`/${locale}/admin/clients/create`}>
      ...
      <DataTableEngine
        ...
        onView={(item) => router.push(`/${locale}/admin/clients/${item.id}`)}
        onEdit={(item) => router.push(`/${locale}/admin/clients/${item.id}/edit`)}
        ...
        bulkActions={[
          {
            ...
            onClick: async (selected) => {
              if (confirm(`Delete ${selected.length} selected clients?`)) {
                await Promise.all(selected.map((item) => deleteClient.mutateAsync(item.id)));
              }
            },
          },
        ]}
