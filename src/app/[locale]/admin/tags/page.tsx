...
import { useDeleteTag, useTags, tagTableColumns } from "@/features/tag";
...
export default function AdminTagsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Confirm Delete")) await deleteTag.mutateAsync(id);
  };

  if (error) {
    return <StateScreen state="error" title="Unable to load tags" compact />;
  }
  ...
  return (
    <div className="ui-reveal-up w-full space-y-6 select-none">
      <AdminPageBanner
        ...
        description={<I18n>Manage content tags for better discovery and filtering.</I18n>}
        actions={
          <Link href={`/${locale}/admin/tags/create`}>
      ...
      <DataTableEngine
        ...
        onView={(item) => router.push(`/${locale}/admin/tags/${item.id}`)}
        onEdit={(item) => router.push(`/${locale}/admin/tags/${item.id}/edit`)}
        ...
        bulkActions={[
          {
            ...
            onClick: async (selected) => {
              if (confirm(`Delete ${selected.length} selected tags?`)) {
                await Promise.all(selected.map((item) => deleteTag.mutateAsync(item.id)));
              }
            },
          },
        ]}
